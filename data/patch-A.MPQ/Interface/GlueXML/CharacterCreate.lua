...
function CharacterCreateEnumerateRaces(...)
	...
	-- Keep the race grid in a stable, Blizzard-like order instead of trusting
	-- GetAvailableRaces() enumeration order.  raceIndex stores the REAL client
	-- selection index, while the button number is only the visual grid slot.
	local raceOrder = {
		HUMAN = 1, DWARF = 2, NIGHTELF = 3, GNOME = 4,
		DRAENEI = 5, WORGEN = 6, HIGHELF = 7, DARKIRONDWARF = 8,
		ORC = 9, SCOURGE = 10, TAUREN = 11, TROLL = 12,
		BLOODELF = 13, GOBLIN = 14, MAGHAR = 15, OGRE = 16,
	};
	local races = {};
	local realIndex = 1;
	for i=1, select("#", ...), 3 do
		...
		table.insert(races, {
			...
			enabled = select(i+2, ...),
			raceIndex = realIndex,
			order = raceOrder[strupper(fileString or "")] or (100 + realIndex),
		});
		realIndex = realIndex + 1;
	end
	table.sort(races, function(a, b) return a.order < b.order; end);
	...
	for visualIndex, entry in ipairs(races) do
		...
		button.raceIndex = entry.raceIndex;
		button:Show();
		if ( entry.enabled == 1 ) then
			button.enable = true;
			...
			button.name = entry.name;
			button.tooltip = _G[strupper((entry.fileString or "").."_DISABLED")];
		end
	end
